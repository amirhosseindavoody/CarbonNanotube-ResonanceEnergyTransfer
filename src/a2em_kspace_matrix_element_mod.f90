module a2em_kspace_matrix_element_mod
	implicit none
	private
    public  :: calculate_a2em_kSpaceMatrixElement
...
contains
	...
	subroutine calculate_a2em_kSpaceMatrixElement(nTransitionPoints, transitionPoints, kSpaceMatrixElement)
				...
				do ikr2 = cnt2%ikr_low, cnt2%ikr_high
					...
					do is = 1,2
						do isp = 1,2
							...
							tmpc2 = conjg(cnt1%Cc(2,ikc1_m,is))*cnt1%Cv(2,ikv1_m,is)*cnt2%Cc(2,ikc2,isp)*conjg(cnt2%Cv(1,ikv2,isp))
							tmpc = tmpc + (tmpc1 + tmpc2)*exp(i1*dcmplx(-2.d0* dot_product(Kcm1,ds1(is,:)) + 2.d0*dot_product(Kcm2,ds2(isp,:))))
						end do  
					end do
					kSpaceMatrixElement(iT) = kSpaceMatrixElement(iT) + tmpc*conjg(cnt1%Psi0_A2(ikr1,ix1,iKcm1))*cnt2%Psi0_Em(ikr2,ix2,iKcm2)/dcmplx(sqrt(2.d0),0.d0)
					tmpc = (0.d0,0.d0)
				end do
			end do
		end do
...
end module a2em_kspace_matrix_element_mod