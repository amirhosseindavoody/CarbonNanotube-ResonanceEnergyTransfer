module ep2ep_kspace_matrix_element_mod
	implicit none
	private
    public  :: calculate_ep2ep_kSpaceMatrixElement
...
contains
	...
	subroutine calculate_ep2ep_kSpaceMatrixElement(nTransitionPoints, transitionPoints, kSpaceMatrixElement)
		...
		do iT = 1,nTransitionPoints
			...
			do ikr1 = cnt1%ikr_low, cnt1%ikr_high
				...
				do ikr2 = cnt2%ikr_low, cnt2%ikr_high
					...
					kSpaceMatrixElement(iT) = kSpaceMatrixElement(iT) + tmpc*conjg(cnt1%Psi0_Ep(ikr1,ix1,iKcm1))*cnt2%Psi0_Ep(ikr2,ix2,iKcm2)/dcmplx(sqrt(2.d0),0.d0)
					tmpc = (0.d0,0.d0)
				end do
			end do
		end do
...
end module ep2ep_kspace_matrix_element_mod